...
| <a name="input_configure_loadbalancer_ssl"></a> [configure\_loadbalancer\_ssl](#input\_configure\_loadbalancer\_ssl) | Enable SSL, and configure the loadbalancer to use the certificate | <pre>object({<br>    enable_ssl               = bool<br>    dns_zone_id              = string<br>    validation_method        = string<br>    https_record_name        = string<br>    https_record_domain_name = string<br>  })</pre> | <pre>{<br>  "dns_zone_id": "",<br>  "enable_ssl": false,<br>  "https_record_domain_name": "",<br>  "https_record_name": ""<br>}</pre> | no |
